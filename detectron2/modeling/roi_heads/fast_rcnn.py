...
class SplitFastRCNNOutputLayers(FastRCNNOutputLayers):
    @configurable
    def __init__(
            self,
            input_shape: ShapeSpec,
        ...
        if isinstance(input_shape_cls, int):  # some backward compatibility
            input_shape_cls = ShapeSpec(channels=input_shape_cls)
        self.num_classes = num_classes
        input_size_cls = input_shape_cls.channels * (input_shape_cls.width or 1) * (input_shape_cls.height or 1)

        if isinstance(input_shape_loc, int):  # some backward compatibility
            input_shape_loc = ShapeSpec(channels=input_shape_loc)
        input_size_loc = input_shape_loc.channels * (input_shape_loc.width or 1) * (input_shape_loc.height or 1)

        # prediction layer for num_classes foreground classes and one background class (hence + 1)
        self.cls_score = Linear(input_size_cls, num_classes + 1)
        num_bbox_reg_classes = 1 if cls_agnostic_bbox_reg else num_classes
        box_dim = len(box2box_transform.weights)
        self.bbox_pred = Linear(input_size_loc, num_bbox_reg_classes * box_dim)
